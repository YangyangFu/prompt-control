#!/usr/bin/env python3
"""
This is a simple server that exposes the OpenAI Gym API over HTTP.
The code is originally from: https://github.com/openai/gym-http-api/blob/master/gym_http_server.py.
The following modifications are made:
- modified to work with gymnasium >= 0.28.1
"""
...
class Envs(object):
    ...
    def create(self, env_id, render_mode="human", seed=None):
        try:
            env = gym.make(env_id, render_mode=render_mode)
            # TODO: this is discarded in the new verion of gymnasium
            if seed:
                env.seed(seed)
    ...
    def reset(self, instance_id, seed=None):
    ...
    def get_observation_space_contains(self, instance_id, j):
        ...
        for key, value in j.items():
            ...
            if json.dumps(info[key]) != json.dumps(value):
                print('Values for "{}" do not match. Passed "{}", Observed "{}".'.format(
                    key, value, info[key]))
                return False
        return True
    ...
    def _get_space_properties(self, space):
        ...
        if info['name'] == 'Discrete':
            ...
            # also make it flat.
            info['low'] = [(float(x) if x != -np.inf else -1e100)
                           for x in np.array(space.low).flatten()]
            info['high'] = [(float(x) if x != +np.inf else +1e100)
                            for x in np.array(space.high).flatten()]
        elif info['name'] == 'HighLow':
            info['num_rows'] = int(space.num_rows)
            info['matrix'] = [((float(x) if x != -np.inf else -1e100) if x != +
                               np.inf else +1e100) for x in np.array(space.matrix).flatten()]
        return info
    ...
    def monitor_start(self, instance_id, directory):
        ...
        self.envs[instance_id]=RecordVideo(
            env, directory)
...
def get_optional_param(json, param, default):
    ...
    value = json.get(param, None)
    if (value is None) or (value == '') or (value == []):
        logger.info("An optional request parameter '{}' had value {} and was replaced with default value {}".format(
            param, value, default))
        value = default
    return value
...
def env_step(instance_id):
    ...
    [obs_jsonable, reward, done, truncated, info] = envs.step(instance_id, action, render)
    return jsonify(observation=obs_jsonable,
                   reward=reward, done=done, truncated=truncated, info=info)
...
def env_monitor_start(instance_id):
    ...
    envs.monitor_start(instance_id, directory)
    return ('', 204)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-l', '--listen', help='interface to listen to', default='127.0.0.1')
    parser.add_argument('-p', '--port', default=5000,
                        type=int, help='port to bind to')
    args = parser.parse_args()

    # this is to run a specific game in the server: 
    # the game can also be defined by the client using the API "env_create()"
    env_id = "CartPole-v1"
    in_id = envs.create(env_id)
    envs.reset(in_id)
